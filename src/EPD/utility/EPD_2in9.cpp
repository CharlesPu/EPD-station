...
#define EPD_W21_CS_0 digitalWrite(epd_gpio_2in9->CS_Pin, LOW)
#define EPD_W21_CS_1 digitalWrite(epd_gpio_2in9->CS_Pin, HIGH)

#define EPD_W21_DC_0 digitalWrite(epd_gpio_2in9->DC_Pin, LOW)
#define EPD_W21_DC_1 digitalWrite(epd_gpio_2in9->DC_Pin, HIGH)

#define EPD_W21_RST_0 digitalWrite(epd_gpio_2in9->RES_Pin, LOW)
#define EPD_W21_RST_1 digitalWrite(epd_gpio_2in9->RES_Pin, HIGH)

#define isEPD_W21_BUSY digitalRead(epd_gpio_2in9->BUSY_Pin)
// 296*128///////////////////////////////////////
...
{
  DEV_SPI_WriteByte(epd_gpio_2in9, value);
...
void EPD_SetRAMValue_BaseMap(const unsigned char *datass)
{
  ...
  datass_flag = datass;
  // FULL update
  ////////////////////////// can update all first, or not
  EPD_HW_Init();
  EPD_W21_WriteCMD(0x24); // Write Black and White image to RAM
  for (i = 0; i < ALLSCREEN_GRAGHBYTES; i++)
  {
    if(datass!=nullptr)EPD_W21_Writedatas(pgm_read_byte(&datass[i]));else EPD_W21_Writedatas(0xff);
  }
  EPD_Update();
  delay(100);
...
void EPD_Dis_Part(unsigned int x_start, unsigned int y_start, const unsigned char *datass, unsigned int PART_LINE, unsigned int PART_COLUMN)
{
  ...
  y_end1 = 0;
  y_end2 = y_start - PART_COLUMN - 1; // note！！减号 -PART_COLUMN  ！！！！！！
  if (y_end2 >= 256)
  {
    y_end1 = y_end2 / 256;
    y_end2 = y_end2 % 256;
  }
  // printf("aaa %02x %02x %02x %02x\r\n", y_start1,y_start2,y_end1,y_end2);
  EPD_W21_WriteCMD(0x44);       // set RAM x address start/end, in page 35
  EPD_W21_Writedatas(x_start);  // RAM x address start at 00h;
  EPD_W21_Writedatas(x_end);    // RAM x address end at 0fh(15+1)*8->128
  EPD_W21_WriteCMD(0x45);       // set RAM y address start/end, in page 35
